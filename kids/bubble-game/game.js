(function () {
  const LEVELS = [
    {
      ...
      description: "Самый мягкий режим без таймера.",
  ...
  const STORAGE_KEY = "babyBubbleGameSettingsV2";
  const MAX_CHARGE = 4;
  ...
  function renderChoiceList(listElement, items, activeId, onSelect) {
    ...
    items.forEach((item) => {
      ...
      if (listElement === locationList && getLevelProgress(item.id).completed) {
        button.classList.add("is-completed");
      }
      if (listElement === locationList && currentLocation().recommendedNext === item.id && item.id !== activeId) {
  ...
  function renderMenus() {
    renderChoiceList(locationList, LOCATIONS, selectedLocationId, (location) => {
      selectedLocationId = location.id;
      selectedToolId = location.defaultToolId;
    });
  ...
  function resizeCanvas() {
    ...
    pointer.x = pointer.x || width / 2;
    pointer.y = pointer.y || height / 2;
  }
  ...
  function effectiveToolRadius() {
    ...
    const scale = input.left && isSuperLocation() ? 1 + Math.min(MAX_CHARGE, input.charge) : 1;
    return tool.radius * (isSuperLocation() ? scale : 1);
  }
  ...
  function resetRound() {
    ...
    finalScore = 0;
    elapsedTime = 0;
    roundTime = location.roundSeconds;
  ...
  const SoundManager = {
    ...
    effectsGain() {
      return Math.max(0, Math.min(1, effectsVolume));
    },

    superGain() {
      return Math.max(0, Math.min(1, effectsVolume * superVolume));
    },
    ...
    playFile(option, gain, fallback) {
      ...
      if (playPromise?.catch) {
        playPromise.catch(() => {
          ...
          if (!this.playFile(retry, gain, fallback)) {
            fallback?.();
          }
        });
      }
    ...
    startLoopFile(option, gain, fallback) {
      const sources = this.getSources(option).filter((source) => !this.unavailableFiles.has(source));
      const source = sources[0];
      if (!source) {
        return null;
      ...
      if (playPromise?.catch) {
        playPromise.catch(() => {
          this.unavailableFiles.add(source);
          const retry = { ...option, files: sources.slice(1) };
          const nextElement = this.startLoopFile(retry, gain, fallback);
          if (nextElement && chargeAudio?.mode === "file") {
  ...
  function playToneSequence(steps, volume) {
    ...
    master.gain.setValueAtTime(Math.max(0.001, volume * SoundManager.effectsGain()), now);
  ...
  function playPopTone(audio, options) {
    ...
    gain.gain.exponentialRampToValueAtTime(options.volume * SoundManager.effectsGain(), now + 0.012);
  ...
  function playNoiseBurst(audio, options) {
    ...
    gain.gain.setValueAtTime(0.001, now);
    gain.gain.exponentialRampToValueAtTime((options.volume || 0.08) * SoundManager.effectsGain(), now + 0.006);
  ...
  function playPopSound(soundId, force) {
    ...
    if (SoundManager.getSources(option).length > 0 && SoundManager.playFile(option, SoundManager.effectsGain() * 0.7, () => playPopSound(option.fallback, true))) {
      return;
    }
  ...
  function startChargeSound(soundId) {
    ...
    if (SoundManager.getSources(selectedOption).length > 0) {
      const loopElement = SoundManager.startLoopFile(selectedOption, SoundManager.superGain() * 0.75, () => {
    ...
    const config = getChargeConfig(selectedOption.file ? selectedOption.fallback : selectedOption.id);
    const master = audio.createGain();
    const filter = audio.createBiquadFilter();
    filter.type = "lowpass";
  ...
  function updateChargeSound() {
    ...
    if (chargeAudio.mode === "file") {
      ...
      chargeAudio.element.volume = Math.max(0, Math.min(1, SoundManager.superGain() * (0.32 + ratio * 0.5)));
      return;
    }
    ...
    const volume = (config.volumeBase + ratio * config.volumeRise) * SoundManager.superGain();
  ...
  function playWaveSound() {
    playToneSequence([220, 360, 580, 820], 0.22);
  }

  function playBurstSound() {
    playToneSequence([920, 680, 440, 260], 0.28);
  }

  function playFreezeSound() {
    playToneSequence([640, 520, 420, 360], 0.2);
  }

  function playStarRainSound() {
    playToneSequence([520, 760, 1020, 1320], 0.24);
  }
  ...
  function updateMobileControlsVisibility() {
    const showControls = running && isTouchDevice && mobileSuperButtonsToggle.checked && currentLocation().superPowers;
  ...
  function addScore(points) {
    score += points;
    const targetScore = currentLocation().targetScore || 0;
    if (targetScore > 0 && score >= targetScore) {
      finishRound("Цель достигнута");
    }
  }
  ...
  function handleWandCollisions() {
    ...
    const hitRadius = effectiveToolRadius() * 0.72;
  ...
  function updateRound(delta) {
    ...
    if (currentLocation().roundSeconds > 0) {
      roundTime -= delta;
      if (roundTime <= 0) {
        roundTime = 0;
        finishRound("Время вышло");
        return false;
      }
    }
  ...
  function drawStarWand(x, y, radius) {
    ...
    const starX = x + radius * 1.08;
    const starY = y + radius * 1.08;
    ...
    ctx.lineWidth = Math.max(8, radius * 0.13);
    ctx.beginPath();
    ctx.arc(x, y, radius * 0.72, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.shadowBlur = 12;
    starPath(ctx, starX, starY, radius * 0.42, radius * 0.25, 5, -Math.PI / 2);
    ...
    ctx.shadowBlur = 0;
    starPath(ctx, starX, starY, radius * 0.24, radius * 0.13, 5, -Math.PI / 2);
  ...
  function drawWand() {
    const tool = currentTool();
    const radius = effectiveToolRadius();
    const x = pointer.x;
    const y = pointer.y;

    if (tool.style === "star" || currentLocation().style !== "soft") {
      drawStarWand(x, y, radius);
    } else {
  ...
  function drawHud() {
    const location = currentLocation();
    const timerText = location.roundSeconds > 0 ? formatTime(roundTime) : formatTime(elapsedTime);
    ...
    if (location.targetScore > 0) {
      ctx.font = "700 15px Arial, Helvetica, sans-serif";
      ctx.fillStyle = "rgba(255, 255, 255, 0.88)";
      ctx.fillText(`цель ${location.targetScore}`, 52, 79);
      ctx.font = "700 22px Arial, Helvetica, sans-serif";
    }
  ...
  function tick(timestamp) {
    ...
    const roundIsActive = updateRound(rawDelta);
  ...
  function handleMouseDown(event) {
    if (!running || shouldIgnoreGamePointer(event)) {
      return;
    }
  ...
  function handleTouchStart(event) {
    if (!running || shouldIgnoreGamePointer(event) || event.targetTouches.length === 0) {
      return;
    }
    event.preventDefault();
